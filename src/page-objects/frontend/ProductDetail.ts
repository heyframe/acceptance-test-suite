...
export class ProductDetail implements PageObject {
    ...
    constructor(page: Page) {
        ...
        this.addToCartButton = page.getByRole('button', { name: translate('frontend:product:addToCart') });
        this.quantitySelect = page.getByLabel(translate('frontend:product:quantity'), { exact: true });
        ...
        this.offCanvasCartTitle = page.getByText(translate('frontend:checkout:cart.shoppingCart'), { exact: true });
        this.offCanvasCart = page.getByRole('dialog');
        this.offCanvasCartGoToCheckoutButton = page.getByRole('link', { name: translate('frontend:checkout:cart.goToCheckout') });
        ...
        this.reviewsTab = this.page.getByRole('tab', { name: translate('frontend:product:review.tabTitle') });
        ...
        this.reviewEmptyListingText = this.page.getByText(translate('frontend:product:review.emptyText'));
        this.reviewLoginForm = this.page.locator('.product-detail-review-login');
        this.forgottenPasswordLink = this.page.getByRole('link', { name: translate('frontend:account:login.forgotPassword') });
        this.reviewLoginButton = this.page.getByRole('button', { name: translate('frontend:account:login.logIn') });
        this.reviewEmailInput = this.page.getByLabel(translate('frontend:account:login.email'));
        this.reviewPasswordInput = this.page.getByLabel(translate('frontend:account:login.password'));
        this.reviewForm = this.page.locator('.product-detail-review-form');
        this.reviewTitleInput = this.page.getByLabel(translate('frontend:product:review.title'));
        this.reviewReviewTextInput = this.page.getByLabel(translate('frontend:product:review.text'));
        ...
        this.reviewSubmitMessage = this.page.getByText(translate('frontend:product:review.submitMessage'));
    }
