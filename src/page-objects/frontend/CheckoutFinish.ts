...
export class CheckoutFinish implements PageObject {
    ...
    constructor(page: Page) {
        this.page = page;
        this.headline = page.getByRole('heading', { name: translate('frontend:checkout:finish.thankYouForOrder') });
        this.orderNumberText = page.getByText(this.orderNumberRegex);
        this.grandTotalPrice = page.locator(`dt:has-text("${translate('frontend:checkout:finish.grandTotal')}") + dd`);
        this.taxPrice = page.locator(`dt:text-matches('${translate('frontend:checkout:finish.plusVat')} [0-9]\\+\\?${translate('frontend:checkout:finish.vatSuffix')}') + dd`);
